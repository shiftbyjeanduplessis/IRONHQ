...
export default async function CoachAthleteDetailPage({ params }: { params: Promise<{ athleteId: string }> }) {
    ...
    return (
      <main className="mx-auto max-w-4xl p-4 md:p-6">
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-6 text-sm text-amber-900">
          Coach identity is required before athlete detail can be shown.
        </div>
      </main>
    );
  }
  ...
  const [continuity, recentSessions, programTemplates] = await Promise.all([
  ...
  return (
    <main className="mx-auto max-w-4xl p-4 md:p-6">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        ...
        <Link href="/coach/roster" className="text-sm font-medium text-neutral-700 underline underline-offset-4">Back to roster</Link>
      </div>
      ...
      <section className="mb-6 rounded-2xl border bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="text-sm text-neutral-600">
            ...
            <p className="mt-1">Coach scope: active</p>
          </div>
        </div>
      </section>

      <section className="mb-6 rounded-2xl border bg-white p-4 shadow-sm" id="assignment">
      ...
      <section className="rounded-2xl border bg-white p-4 shadow-sm">
        ...
        <div className="mt-4 space-y-3">
          {recentSessions.length === 0 ? (
            <p className="text-sm text-neutral-600">No sessions yet in this coach scope.</p>
