    ...
    return (
      <main className="mx-auto max-w-4xl p-4 md:p-6">
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-6 text-sm text-amber-900">
          Coach identity is required before coach sessions can be shown.
        </div>
      </main>
    );
  }
    ...
    <main className="mx-auto max-w-5xl p-4 md:p-6">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-neutral-600">Coach-scoped recent sessions.</p>
        </div>
      ...
      <section className="mb-5 rounded-2xl border bg-white p-4 shadow-sm">
        <form className="grid gap-3 md:grid-cols-[1fr_auto_auto] md:items-end" method="get">
          <label className="space-y-2"><span className="text-sm font-medium text-neutral-800">Search athlete or session</span><input name="q" defaultValue={q} placeholder="Search athlete, program, session title" className="w-full rounded-xl border px-3 py-2 text-sm" /></label>
          <label className="space-y-2"><span className="text-sm font-medium text-neutral-800">Status</span><select name="status" defaultValue={status} className="rounded-xl border px-3 py-2 text-sm"><option value="all">All statuses</option><option value="completed">Completed</option><option value="missed">Missed</option><option value="in_progress">In Progress</option><option value="pending">Pending</option></select></label>
          <div className="flex gap-2"><button type="submit" className="rounded-xl bg-black px-4 py-2 text-sm font-medium text-white">Apply</button><Link href="/coach/sessions" className="rounded-xl border border-neutral-300 px-4 py-2 text-sm text-neutral-800">Clear</Link></div>
        </form>
        <p className="mt-3 text-xs text-neutral-500">Showing {sessions.length} recent sessions in the active coach scope.</p>
      </section>
      <div className="space-y-3">
        {sessions.length === 0 ? <div className="rounded-2xl border bg-white p-4 text-sm text-neutral-600 shadow-sm">No coach-scoped sessions match the current filters.</div> : sessions.map((session) => (
