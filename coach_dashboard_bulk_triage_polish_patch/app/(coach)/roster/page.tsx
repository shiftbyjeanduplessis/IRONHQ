...
export default async function CoachRosterPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const hydratedRows = await Promise.all(assignments.map(async (assignment) => {
    ...
    const attention = missedCount > 0 || openNotes > 0 || continuity.kind === 'missed' || !assignment.programName;
    const primaryAction = missedCount > 0 || continuity.kind === 'missed'
      ? { href: `/coach/sessions?status=missed&q=${encodeURIComponent(assignment.athleteName)}`, label: 'Review missed sessions' }
      : openNotes > 0
        ? { href: `/coach/athletes/${assignment.athleteId}#coach-notes`, label: 'Open follow-ups' }
        : !assignment.programName
          ? { href: `/coach/athletes/${assignment.athleteId}#assignment`, label: 'Assign program' }
          : { href: `/coach/athletes/${assignment.athleteId}`, label: 'Open athlete' };
    return { ...assignment, continuity, missedCount, lastCompletedAt, nextSessionTitle, openNotes, attention, primaryAction };
  }));

  const rows = hydratedRows.filter((row) => {
    const haystack = [row.athleteName, row.programName ?? '', row.nextSessionTitle].join(' ').toLowerCase();
    return (!q || haystack.includes(q)) && matchesStatus(row.continuity.kind as ContinuityKind, status) && (!onlyNeedsAttention || row.attention) && (!onlyOpenNotes || row.openNotes > 0);
  });

  const counts = {
    total: hydratedRows.length,
    shown: rows.length,
    needsAttention: hydratedRows.filter((row) => row.attention).length,
  };
        ...
        {rows.length === 0 ? (
          ...
          <section key={row.id} className="rounded-2xl border bg-white p-4 shadow-sm">
            ...
            <div className="mt-4 flex flex-wrap gap-3 text-sm">
              <Link href={row.primaryAction.href} className="rounded-xl bg-black px-3 py-2 font-medium text-white">{row.primaryAction.label}</Link>
